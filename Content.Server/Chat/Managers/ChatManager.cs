...
namespace Content.Server.Chat.Managers
{
    ...
    internal sealed partial class ChatManager : IChatManager
    {
        ...
        public void SendAdminAnnouncement(string message, AdminFlags? flagBlacklist, AdminFlags? flagWhitelist)
        {
            ...
            }).Where(p => _adminManager.GetAdminData(p)?.HasFlag(AdminFlags.Adminhelp) ?? false).Select(p => p.Channel);
        ...
        private void SendAdminChat(ICommonSession player, string message)
        {
            ...
            var clients = _adminManager.ActiveAdmins.Where(p => _adminManager.GetAdminData(p)?.HasFlag(AdminFlags.Adminhelp) ?? false).Select(p => p.Channel);
